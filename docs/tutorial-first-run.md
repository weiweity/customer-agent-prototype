...
过敏售后切片 1：输入 `过敏了怎么办`（或 `用了露芷面膜过敏了怎么办`）。成功标志：Top 3 结果标题和卡片之间出现「过敏可走售后流程 · 建议先要凭证」和按钮「打开过敏售后流程」。点按钮后出现独立可拖动的 SOP 窗，首屏是要照片 / 批次的凭证话术。Query 复制仍约 900ms 收起查询，不会关掉 SOP；SOP 复制只反馈 1.2s，不进入 Query `COPIED`。`怎么用` + `过敏`、空结果、少发 / 长痘都不应出现 SOP 按钮。
