...
过敏售后切片 1：输入 `过敏了怎么办`（或 `用了露芷面膜过敏了怎么办`）。成功标志：Top 3 结果标题和卡片之间出现「过敏可走售后流程 · 建议先要凭证」和按钮「打开过敏售后流程」。点按钮后出现独立可拖动的 SOP 窗，首屏是要照片 / 批次的凭证话术。Query 复制仍约 900ms 收起查询，不会关掉 SOP；SOP 复制只反馈 1.2s，不进入 Query `COPIED`。`怎么用` + `过敏`、空结果、少发 / 长痘都不应出现 SOP 按钮。SOP 卡片没有「话术不准」。

Query Top 3 每张卡有发丝「话术不准」。点后该卡显示「已记录，待话术师核实」，复制仍是「已复制」，不得出现已处理 / 已发送 / 已上报。收起查询后本地记录丢弃。
...
「内容与发布」可选择本地 CSV，解析成待审核草稿；Publish 保持禁用。二进制 xlsx 应失败关闭。售后过敏树说明是 DEMO 样例，不接本页上传。

「话术优化待办」顶部一条横幅列出 2 条待处理 P0。点选同步详情。这是会话内合成演练，刷新即丢，不是飞书推送或正式 HTTP 待办。
