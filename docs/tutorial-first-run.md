...
过敏售后切片 1：输入 `过敏了怎么办`（或 `用了露芷面膜过敏了怎么办`）。成功标志：Top 3 结果标题和卡片之间出现「过敏可走售后流程 · 建议先要凭证」和按钮「打开过敏售后流程」。点按钮后出现独立可拖动的 SOP 窗，首屏是要照片 / 批次的凭证话术。Esc / × 收起后，同一结果应改成「继续过敏售后流程」。Query 复制仍约 900ms 收起查询，不会关掉 SOP；SOP 复制只反馈 1.2s，不进入 Query `COPIED`。`怎么用` + `过敏`、空结果、少发 / 长痘都不应出现 SOP 按钮。SOP 卡片没有「话术不准」，也没有 Query 排名「1」。选严重程度时有一句可见提示；凭证步才有「不建单、不指令仓配」。
...
重新点狐狸头打开查询，再点胶囊右侧的线框 Dashboard 图标（`aria-label` 为「打开运营工作台」）。已登录则直接开工作台。已接 loopback 合成栈且未登录时，这个图标会先打开「登录」窗，成功后才开工作台；关掉登录窗则不打开。登录窗分段只有 **飞书 / 账号**：飞书会打开系统浏览器，不在本窗加载授权页；账号表单仍在本窗。S0 日常路径没有 loopback origin，点登录不可用，不要把它当成真实飞书。狐狸 / 查询右键、「打开运营工作台」、系统 Tray / 应用菜单、macOS 程序坞仍直接打开 / 恢复 Dashboard，不经过胶囊登录门。程序坞点击不会展开查询。
