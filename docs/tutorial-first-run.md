...
重新点狐狸头打开查询，再点胶囊右侧的线框 Dashboard 图标（`aria-label` 为「打开运营工作台」）。未登录时这个图标会先打开「登录」窗，成功后才开工作台；关掉登录窗则不打开。已登录则直接开。狐狸 / 查询右键、「打开运营工作台」、系统 Tray / 应用菜单、macOS 程序坞仍直接打开 / 恢复 Dashboard，不经过胶囊登录门。程序坞点击不会展开查询。
