...
本页是 `DEV-M0-W3～W5` 基线与 `DEV-M1-W1` 当前可执行 API 配置的 SSOT。产品化总路线继续由获批计划拥有；本页只记录本机 mock-auth、runtime/admin capability pool 与密钥启动边界，不授权真实身份、真实数据、部署或 runtime activation。
...
| `AUTH_MODE` | 必填；当前只接受 `mock` | 只输出字段名和稳定拒绝原因 |
...
| `DB_POOL_MAX` | 可省略，runtime 默认 `18`；只接受 `1..20` 正整数 | 不进入公开 config 或响应 |
| `CONTENT_ADMIN_DB_POOL_MAX` | 可省略，admin 默认 `2`；只接受 `1..20`，且两个 pool 合计不得超过 `20` | 同上 |
