...
本页是 `DEV-M0-W3～W5` 基线与 `DEV-M1-W0～W5` 当前可执行 API 配置的 SSOT。产品化总路线继续由获批计划拥有；本页只记录本机 mock-auth、runtime/admin capability pool、Search + Events 与密钥启动边界，不授权真实身份、真实数据、桌面接线、部署或 runtime activation。
