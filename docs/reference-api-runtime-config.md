...
| `CONTENT_OBJECT_STORE_DIR` | 可省略；提供时必须是本机绝对目录，供 CSV/XLSX 不可变对象落盘。未设置时导入路由 503，storage readiness 保持 `not_ready`，不增加连接池。已设置时 `/ready` 的 storage 必须实际 persist/read/verify | 只存在于私有 bootstrap config，不回显原值 |
