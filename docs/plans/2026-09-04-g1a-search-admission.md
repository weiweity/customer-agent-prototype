...
| T4 | **BLOCKED · RISK REVALIDATION REQUIRED** | 历史静态验收保留；33 条风险组合须逐条审定，正式批准 0/33，未齐不装包 | 历史 `EVD-G1A-DATA-01`、`EVD-G1A-EVALSET-01` 不替代新风险批准 | 不属于代码提交 |
| T5 | **ATTEMPTED / BLOCKED · NOT_EVALUATED** | Attempt06/07 未形成评测报告；风险校验修正已合并，先审定并版本化重装，再核验新基线与 dry-run | `EVD-G1A-RUN-07`、`EVD-G1A-CLEANUP-07`；不以合成分数代替真实结果 | T3、T4 后 |
...
### 6.2 T4 受控准备清单（历史静态验收保留，当前风险须重新审定）
...
2. 在仓外完成 33 条来源绑定的真实风险审定；高风险/冲突保持不同主体双审，候选分类和结构校验不充当签名。
3. 审定齐备后生成版本化新包、外部锚点与证据；保留原包，不自动延长有效期，不覆盖历史尝试。
