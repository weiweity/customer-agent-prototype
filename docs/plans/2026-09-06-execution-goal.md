...
| 桌面语义检索 | 本机冻结，见 [冻结点](2026-09-10-macos-semantic-query-freeze.md) 与 [检索参考](../reference-desktop-retrieval.md)。有 hydrate 时不打 leftover `/v1/search`。不要 `stack start`。仓外 `questions[]` 由 `pnpm retrieval:questions` 入库。下一动作：第二路 embedding、hydrate 自动对齐 `content_current` |
