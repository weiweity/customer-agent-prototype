...
> **状态：** 已刷新（`feat/p10-linux-xdg-userdata`）。相对 `origin/main` `0ab267b`。
...
8. **P10 Linux local-unsigned + CI + XDG 尖端** `feat/p10-linux-xdg-userdata`（含 `9a1b519` UNSIGNED AppImage artifact、Linux userData 跟随 `XDG_CONFIG_HOME`）。Overlay smoke 仍启动打包 `out/`，不是 AppImage。
...
P10 Linux 打包中间的 `feat/p10-package-linux-local` … `feat/p10-linux-e2e-deps` `cfc4bed` … `feat/p10-linux-ci-artifact` `9a1b519` 已被 `feat/p10-linux-xdg-userdata` 包含。
