...
- P3：16/16项引导式自述已记录（6澄清、10转交类），负责人明确调整本次证据口径，接受处理判断验证；不是实际执行或独立盲测。
- P4：负责人已明确给出业务/QA意见并要求签发；T6记录完成，证据ID `EVD-G1A-T6-GUIDED-20260908`。治理仓《2026-09-08_G1a引导式验证口径调整与T6签发》拥有决定；签发摘要 `857be8d8bedb39c996bd4b4768ad28b2e62812825749d7dc9b9d6845392e61d0`。原机器 `EVALUATED_NOT_SIGNED / REVIEW_REQUIRED`、下游 `NOT_EVALUATED` 保持历史事实。
- 本次未测：候选展示/复制、平台与商品隔离的界面接入、失效话术拦截、敏感资料权限和实际转交。P1–P4仅按调整后范围收口，DEV-M2及正式接入仍未批准。
...
- [x] T5：009真实比较完成；16项引导式处理判断已按负责人调整后的口径接受。
- [x] T6：负责人业务/QA复核及签发已记录，范围和未测项见当前状态。
...
VERDICT: P1–P4按明确调整后的引导式验证范围收口；T6不授权正式运行。

**UNRESOLVED DECISIONS:**
- 下一阶段实施范围和开工批准仍待独立明确；既有签发授权不重复询问。
