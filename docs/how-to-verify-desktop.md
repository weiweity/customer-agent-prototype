...
仅开发态显式设置 `CUSTOMER_AGENT_DESKTOP_API_ORIGIN` 与 `CUSTOMER_AGENT_DESKTOP_IDENTITY_ORIGIN` 后运行 `pnpm dev`。两者必须是含端口的 `http://127.0.0.1:端口` origin，分别对应已经启动的 formal-dev API 与本机身份服务（无 `feishu.env` 时是合成 identity；有 `feishu.env` 时是口令服务 `43101`，飞书走官方授权）。桌面 origin **不能**写成 `accounts.feishu.cn`。缺一项、非 loopback 或打包态拒启。无配置继续显式 S0 开发模式；接入模式故障不回退 S0。真飞书本机步骤见 [方案 C how-to](how-to-feishu-and-password-mac.md)。
