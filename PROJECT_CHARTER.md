...
> **生效：** 2026-08-21
> **仓库身份：** 客服 Agent 产品实施仓
> **当前基线：** Menokin 是当前唯一客服试点；`DEV-M0` 已于 2026-08-31 开工，W1 已把现有 Electron 应用机械迁入唯一桌面包 `apps/desktop`，W2 已建立可执行的 `packages/contracts` 合同编译与运行时校验边界，W3 已完成只含 `/health` 的本机 API/config 候选实现与本地验证（尚未提交）；当前桌面产品运行链仍是 `PILOT-S0 · SYNTHETIC` 合成基线
...
- 正式合同快照继续保持 `ddev_authorized=false`、`runtime_activated=false`；W3 候选只新增 loopback Fastify `/health` 与配置拒启矩阵，没有 migration、DB、`/ready`、九业务端口或桌面接线，后续能力仍须按独立切片依次授权和验证。
