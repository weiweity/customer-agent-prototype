  ...
  return startApiWithFactory(
    ...
    async (config, repository, policyAdminRepository, bootstrap) => {
      ...
      if (config.sessionMode === 'product') {
        ...
        if ((config.authMode === 'feishu') !== (bootstrap.productIdentity.kind === 'oidc')) {
          throw new Error('Product identity bootstrap is incomplete');
        }
        const provider = bootstrap.productIdentity.kind === 'oidc'
          ? createOidcIdentityProvider(bootstrap.productIdentity.oidc)
