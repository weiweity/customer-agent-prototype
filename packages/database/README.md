...
`DEV-M0-W4` 的 PostgreSQL 15 控制面深模块。它从已验证的合同 snapshot 确定性生成九个不可变 migration，并把来源、顺序、checksum、session advisory lock、逐 migration 事务、私有账本和精确 catalog/ACL/函数定义后验封装在 `status → plan → apply → verify` 接口内。公共 status/plan 只返回 provenance 元数据，不暴露可执行 SQL。
