...
4. 登录窗：约 `520×420` 的标准系统窗，**只用原生标题栏**，不套 hiddenInset / overlay chrome。标题「合成身份登录」。胶囊点「合成登录」打开；分段文案只有 **飞书 / 账号**，禁止内部 / 外包或其它受众标签。视觉复用查询胶囊 token（浅白玻璃底、`--ink` / `--muted`、淡紫主按钮、11px 次控件、输入井 16/24 与 14px 圆角）。飞书在同一窗口打开授权页，不重绘官方页；账号是管理员下发的口令表单，失败只说「账号或密码不正确」，找回只说联系管理员。独立非持久 session，不与 Query / Dashboard 共享 cookie。关窗或 Esc 是取消，成功以会话交换为准。不改胶囊几何，也不把登录做成胶囊内表单。
5. 售后 SOP 窗：独立 `role=sop` 可拖动窗，宽 600，起壳高 240，hug `240..620`，40px chrome。过敏售后 Top 3 在结果标题和卡片之间给「打开过敏售后流程」；Query 检索与 SOP 可同时开，不自动弹出。`OverlayRole` 仍只有 `fox | query`。SOP 复制只反馈 1.2s，不进入 Query `COPIED`，也不走 `copyAdopt`。切片 1 仅合成过敏树 DEMO，坐席只读。
...
- `EMPTY`：明确「没找到可用话术」。提示先让客户稍等，再联系话术师或运营核实，必要时升级客服经理。可复制合成联系方式或打开合成入口。状态只允许「已打开入口 / 已复制联系方式 / 待核实」，禁止「已转交成功」。S0 未接线时仍说明 Demo 未接通真实话术库。
