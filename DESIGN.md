...
4. 登录窗：约 `520×420` 的标准系统窗，**只用原生标题栏**，不套 hiddenInset / overlay chrome。标题「合成身份登录」。胶囊点「登录」打开；分段文案只有 **飞书 / 账号**，禁止内部 / 外包或其它受众标签。视觉复用查询胶囊 token（浅白玻璃底、`--ink` / `--muted`、淡紫主按钮、11px 次控件、输入井 16/24 与 14px 圆角）。飞书在同一窗口打开授权页，不重绘官方页；账号是管理员下发的口令表单，失败只说「账号或密码不正确」，找回只说联系管理员。独立非持久 session，不与 Query / Dashboard 共享 cookie。关窗或 Esc 是取消，成功以会话交换为准。不改胶囊几何，也不把登录做成胶囊内表单。
...
话术库与内容发布必须分开：前者是只读资产浏览，后者是治理生命周期。话术库读本机 hydrate / 检索索引（与查询胶囊同一份目录），只读、不复制、不发布。VOC / 工单 / 架构页仍是架构模拟，不得因为话术库挂上本机目录就宣称工作台已接正式后端。
