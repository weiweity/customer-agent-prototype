...
**Status:** OPEN · 方案 B 开工（`feat/identity-broker`）。产品做 RP，飞书+账号接到 OIDC 代理（Logto）。见 `docs/plans/2026-09-18-identity-broker.md`。#129 合成口令双通道不作为正路。API 仍只绑 127.0.0.1。未知 `sub` 不自动建号。放宽 loopback / 办公机远端部署仍未授权。
